...
def medico_registar_consulta(request, consulta_id):
    ...
    if request.method == 'POST':
        ...
        dosagem = request.POST.get('dosagem', '').strip()
        instrucoes = request.POST.get('instrucoes', '').strip()
        
        # Atualizar consulta
        consulta.notas_clinicas = notas_medicas
        consulta.estado = 'realizada'
        consulta.save()
        
        # Criar receita se houver medicamento e dosagem
        if medicamento and dosagem:
            ...
            messages.success(request, "Consulta registada e receita médica criada com sucesso!")
        else:
...
@login_required
@role_required('medico')
def medico_criar_receita(request, consulta_id):
    """Criar receita médica para uma consulta"""
    medico = Medico.objects.get(id_utilizador=request.user)
    consulta = get_object_or_404(Consulta, id_consulta=consulta_id, id_medico=medico)
    
    # Verificar se a consulta foi realizada ou está em andamento
    if consulta.estado not in ['realizada', 'confirmada']:
        messages.error(request, "Só é possível criar receitas para consultas confirmadas ou realizadas.")
        return redirect('medico_detalhes_consulta', consulta_id=consulta_id)
    ...
    if request.method == 'POST':
        ...
        if not medicamento or not dosagem:
            messages.error(request, "Medicamento e dosagem são obrigatórios.")
        else:
            try:
                Receita.objects.create(
                    id_consulta=consulta,
    ...
    return render(request, 'medico/criar_receita.html', context)


@login_required
@role_required('medico')
def medico_listar_receitas(request, consulta_id):
    """Listar todas as receitas de uma consulta"""
    medico = Medico.objects.get(id_utilizador=request.user)
    consulta = get_object_or_404(Consulta, id_consulta=consulta_id, id_medico=medico)
    ...
    return render(request, 'medico/listar_receitas.html', context)
...
@login_required
@role_required('medico')
def medico_search_pacientes(request):
    """API endpoint to search patients by name or email"""
    ...
    pacientes = Paciente.objects.select_related('id_utilizador').filter(
        ...
        Q(id_utilizador__email__icontains=query) |
        Q(id_utilizador__n_utente__icontains=query),
        id_utilizador__ativo=True
    ...
    results = [
        {
            'id': p.id_paciente,
            'nome': p.id_utilizador.nome,
            'email': p.id_utilizador.email,
            'n_utente': p.id_utilizador.n_utente,
            'text': f"{p.id_utilizador.nome} - {p.id_utilizador.email}"
        }
        for p in pacientes
    ]
