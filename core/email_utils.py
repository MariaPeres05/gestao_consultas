...
def enviar_email_confirmacao(consulta_id):
    ...
    try:
        consulta = Consulta.objects.select_related(
            'id_paciente__id_utilizador', 'id_medico__id_utilizador', 'id_medico__id_especialidade'
        ).get(id_consulta=consulta_id)
        ...
        recipient_list = [consulta.id_paciente.id_utilizador.email]
...
def enviar_email_cancelamento(consulta_id, motivo_cancelamento=''):
    ...
    try:
        ...
        if hasattr(consulta, 'cancelado_por') and consulta.cancelado_por:
            if consulta.cancelado_por == consulta.id_paciente:
                motivo_texto = "por si"
            elif consulta.cancelado_por == consulta.id_medico:
...
def enviar_lembretes_24h():
    ...
    consultas = Consulta.objects.filter(
        estado='confirmada',
        data_consulta__gte=inicio_janela.date(),
        data_consulta__lte=fim_janela.date(),
    ).select_related('id_paciente__id_utilizador', 'id_medico__id_utilizador', 'id_medico__id_especialidade')
    ...
    for consulta in consultas:
        ...
        if inicio_janela <= data_hora_consulta <= fim_janela:
            try:
                enviar_lembrete(consulta.id_consulta, 'amanhã')
                emails_enviados += 1
            except Exception as e:
                logger.error(f"Erro ao enviar lembrete 24h para consulta {consulta.id_consulta}: {str(e)}")
...
def enviar_lembretes_2h():
    ...
    for consulta in consultas:
        ...
        if inicio_janela <= data_hora_consulta <= fim_janela:
            try:
                enviar_lembrete(consulta.id_consulta, 'daqui a 2 horas')
                emails_enviados += 1
            except Exception as e:
                logger.error(f"Erro ao enviar lembrete 2h para consulta {consulta.id_consulta}: {str(e)}")
